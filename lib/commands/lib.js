...
if (fle) {
	...
	if (fle.compileType === 'babel') {
		spawn(
			...
			[
				...
				'--plugins=' + JSON.stringify([fleResolve('babel-plugin-transform-decorators-legacy'), fleResolve('babel-plugin-add-module-exports'), fleResolve('babel-plugin-transform-runtime')])
			],
			{
				cwd: fleHomePath,
				stdio: 'inherit'
	...
	spawn(
		...
		[
			...
			'--plugins=' + JSON.stringify([fleResolve('babel-plugin-add-module-exports')])
		].filter(a => a),
		{
			cwd: fleHomePath,
			stdio: 'inherit'
