...
if (!opts.page) {
  ...
  inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'lib    ->  JS库工程（ES6）', value: 'lib' },
        { name: 'node   ->  node工程（ES6）', value: 'node' }
      ],
      default: 'react'
    },
    // library添加额外信息
  ...
  inquirer.prompt([
    ...
    if (res.template.indexOf('/h5/') !== -1) {
      res.css = [cdn.css.reset];
      res.prejs = [cdn.js.remResize];
    } else if (res.template.indexOf('/pc/') !== -1) {
      res.css = [cdn.css.reset];
    } else if (res.template === 'custom') {
      res.template = path.join('src', projectName, 'index.html');
      res.filename = `html/${projectName}.html`
    ...
    if (res.platform) {
      ...
      res.uaId = platformItem.id;
      res.icon = platformItem.icon;
      delete res.platform;
    } else {
      res.icon = '/favicon.ico';
    }

    // 表示这个文件可以编译
    res.compiled = true;

    var needExample = res.example;

    delete res.example;

    fs.writeFileSync(path.join(targetPath, 'app.json'), JSON.stringify(res, null, 2), { encoding: 'utf8' });
