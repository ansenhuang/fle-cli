...
var { pkgVersion, wenmanPlatform } = require('../consts');
...
if (!opts.page) {
  ...
  inquirer.prompt([
    {
      ...
      choices: [
        ...
        { name: 'miniprogram ->  小程序工程', value: 'miniprogram' }
      ],
      default: 'react'
    },
    // library添加额外信息
    ...
    },
    {
      type: 'input',
      name: 'appKey',
      message: 'miniproject appKey',
      default: 'wx7a095676ef39b8bc',
      when (a) {
        return a.boilerplate === 'miniprogram'
      }
    },
    {
      type: 'input',
    ...
    // 页面工程
    {
      ...
      default: true,
      when (a) {
        return a.boilerplate !== 'node' && a.boilerplate !== 'miniprogram'
      }
    }
  ]).then(res => {
    ...
    if (res.boilerplate !== 'node') {
      ...
      if (res.boilerplate !== 'miniprogram') {
        fs.writeFileSync(path.join(targetPath, 'fle.json'), JSON.stringify(fleWebpack, null, 2), { encoding: 'utf8' });
      }

      if (res.boilerplate === 'miniprogram') {
        var projTpl = require(path.join(originPath, 'project.config.json'));

        projTpl.appid = res.appKey;
        projTpl.projectname = projectName;
        fs.writeFileSync(path.join(targetPath, 'project.config.json'), JSON.stringify(projTpl, null, 2), { encoding: 'utf8' });
      }
      ...
      if (pkgTpl.dependencies && Object.keys(pkgTpl.dependencies).length) {
        console.log();
        console.log('===========================================');
        spawn.sync(useYarn() ? 'yarn' : 'npm', ['install'], {
          cwd: targetPath,
          stdio: 'inherit'
        });
        console.log('===========================================');
      ...
      if (res.boilerplate !== 'lib' && res.boilerplate !== 'miniprogram') {
        console.log('创建页面：');
        console.log();
  ...
  inquirer.prompt([
    ...
    var app = {
      ...
      filename: `html/${projectName}.html`,
      icon: '/favicon.ico',
      prejs: [],
      css: [],
      js: [],
