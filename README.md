...
* dev和dll可以指定要编译的目录，详细信息请执行：$ fle build -h
* 开启上传功能需要配置密钥等信息，否则无法上传，[cdn参数配置](https://g.hz.netease.com/huangancheng/documents/blob/master/fle/nosConfig.md)【限内部人员查看】，或者也可以在[【这里申请】](https://www.163yun.com/help/documents/15677635979624448)。
...
* src                   # 源码
  ...
  * index               # 示例页面文件，新建页面参考此结构
    * app.json          # 页面信息配置
    * index.js          # 页面入口，也可以在app.json的entry字段定义
    * module.js         # 组件入口，也可以在app.json的module字段定义
...
```
{
  ...
  "externals": {},             # 页面忽略引用的包，转而使用全局变量
  "libExternals": {},          # 组建忽略引用的包，转而使用全局变量

  # 以下为rollup项目配置 #

  "iife": false,               # 是否要导出自执行模块，否则导出es6模块
  "eslint": true,              # 是否开启eslint代码检测
  "port": 5000,                #【dev】端口
  "hot": true,                 #【dev】是否开启热更新
  "https": false,              #【dev】是否开启https
...
```
{
  ...
  "prejs": [],                                                               # 加载js文件外链，插入head
  "js": [],                                                                  # 加载js文件外链，插入body
  "css": [],                                                                 # 加载css文件外链，插入head
...
除以上字段外，可以额外添加字段，在html模版中通过以下方式获取：如额外添加name，则htmlWebpackPlugin.options.name可以获取该值
...
* 采用标准[standard]规则，也可以在项目目录添加.eslintrc文件来重写规则，[标准规则文档](https://github.com/standard/standard/blob/master/docs/RULES-zhcn.md)
